...
')
  ...
  response_number <-  indicator_p %>%
    select(a_total, b_total, posterior) 
  
  ## remove duplicate row
  response_number_unique <- response_number[!duplicated(response_number), ]
  
  # Label "B wins" where posterior > 0.8
  response_number_unique$label <- ifelse(response_number_unique$posterior > 0.8, "B wins", "no winner")
  
  response_number_unique <- response_number_unique %>% 
    rename("Ra" = a_total,
           "Rb" = b_total) %>%
    mutate("Rb-Ra" = Rb- Ra) 
  
  summarise_table <- response_number_unique %>% ## winning threshold
    filter(label =="B wins") %>%
    group_by(Ra)%>%
    summarise("Rb-Ra" = min(Rb- Ra))
  
  ## create final decision table
  final_table <- response_number_unique %>%
    inner_join( summarise_table, by=c("Ra", "Rb-Ra")) ## Inner_join() only keeps observations from x that have a matching key in y
  
  threshold_no <- function(x){
    paste0(r, " < Rb < ", x)
  } 
  
  threshold_bwins <- function(x){
    paste0( " Rb >= ", x)
  } 
  
  
  no_winner <- sapply(final_table$`Rb`, threshold_no) ## no winner column
  a_wins <- rep(paste0( "Rb <= ", r ), length(no_winner)) ## a wins column
  b_wins <- sapply(final_table$`Rb`, threshold_bwins)
  
  decision_table <- data.frame("Ra" = final_table$Ra, 
                               "Arm A wins" = a_wins,
                               "No winner" = no_winner,
                               "Arm B wins" = b_wins)
  ## add first row
  firstrow <- c( paste0("Ra <= ", r),  "_", paste0("Rb <=", r), paste0("Rb >", r))
  
  decision_table2 <- rbind(firstrow, decision_table)
  # library(knitr)
  # 
  # colnames(decision_table2) <- c("Ra", "No winner", "Arm A wins", "Arm B wins")
  # return(decision_table2)
  ## add row up to n
  if(as.numeric(decision_table2$Ra[length(decision_table2$Ra)]) < n){
    
    start <-  as.numeric(decision_table2$Ra[length(decision_table2$Ra)])+ 1 
    extra_row <- data.frame("Ra"= c(start:n),
                            "Arm.A.wins"=rep(paste0("Rb <=", r), n-start+1),
                            "No.winner" = rep( paste0(r, "< Rb"), n-start+1),
                            "Arm.B.wins" = rep( "_", n-start+1)
    )
    
    final_decision_table <- rbind(decision_table2, extra_row)
    colnames(final_decision_table) <- c("Ra", "Arm A wins", "No winner", "Arm B wins")                        
    
  }else{
    final_decision_table<- decision_table2
    colnames(final_decision_table) <- c("Ra", "Arm A wins","No winner",  "Arm B wins") 
  }
